...
func Exit () {
    fmt.Println ("Usage: FastqCount  <input.fastq> [read length] [phred]")
    fmt.Println ("  Read length fixed default: 150;")
...
func main() {
    ...
    if len(os.Args) >= 3 {
        rlen, _ = strconv.Atoi (os.Args[2])
    } else { rlen = 150 }

    if len(os.Args) == 4 {
        phred, _ = strconv.Atoi (os.Args[3])
    } else { phred = 33 }
